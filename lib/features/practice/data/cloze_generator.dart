...
class ClozeGenerator {
  ...
  List<ClozeWord> generate(String text, {
    ...
    // 2. ブラックリスト（暗記対象として不適格な不要語）の除外（pinnedWordsは除外しない）
    allCandidates.removeWhere((wp) =>
        _blacklistedWords.contains(wp.word) &&
        !pinnedClozeWords.contains(wp.word));

    // 3. 2文字の漢字語で接尾辞的な特徴を持つ語の簡易除外（pinnedWordsは除外しない）
    allCandidates.removeWhere((wp) {
      final w = wp.word;
      if (pinnedClozeWords.contains(w)) return false; // ピン留めは除外しない
      if (w.length == 2) {
    ...
    // 優先度ソート：
    // - pinnedClozeWords に含まれる語を最優先
    // - 漢字3文字以上、またはカタカナ4文字以上の語を次優先（重要な専門用語である確率が極めて高いため）
