...
class ClozeGenerator {
  ...
  List<ClozeWord> generate(String text, {
    ...
    // 1. 重要ワード (pinnedClozeWords) を本文全体から直接検索して最優先で確定
    final pinnedClozes = <ClozeWord>[];
    
    // 長いワードを優先してマッチングするため、降順にソート
    final sortedPinnedWords = List<String>.from(pinnedClozeWords)
      ..sort((a, b) => b.length.compareTo(a.length));

    for (final word in sortedPinnedWords) {
      ...
      while (true) {
        ...
        // 既存のマッチ範囲とオーバーラップしているかチェック
        final overlaps = pinnedClozes.any((cw) =>
            (start >= cw.startIndex && start < cw.endIndex) ||
            (end > cw.startIndex && end <= cw.endIndex));
            
        if (!overlaps) {
          pinnedClozes.add(ClozeWord(
            startIndex: start,
            endIndex: end,
            word: word,
        ...
        index += word.length; // 次の検索へ
      }
    }

    // 2. テキスト全体から出現頻度マップを作成（重要用語判定の精度維持のため）
    ...
    // 3. テキストを文（句点、感嘆符、疑問符、改行など）ごとに分割
    ...
    for (final segment in segments) {
      ...
      for (final wp in uniqueCandidates) {
        ...
        final absStart = wp.startIndex + offset;
        final absEnd = wp.endIndex + offset;

        // すでに重要ワード（pinnedClozes）として確保された位置とオーバーラップしていないかチェック
        final overlapsPinned = pinnedClozes.any((cw) =>
            (absStart >= cw.startIndex && absStart < cw.endIndex) ||
            (absEnd > cw.startIndex && absEnd <= cw.endIndex));
        if (overlapsPinned) continue;

        // セグメント内で既に選択された自動生成穴埋め語とオーバーラップしていないかチェック
        final overlapsSelf = segmentSelected.any((cw) =>
            (absStart >= cw.startIndex && absStart < cw.endIndex) ||
            (absEnd > cw.startIndex && absEnd <= cw.endIndex));
    ...
    // 最後に重要ワードと自動生成をマージし、絶対位置順にソートして返却
    final result = [...pinnedClozes, ...selected];
